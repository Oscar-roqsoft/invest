      ...
      <div class="container max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <header class="header flex items-center justify-between mb-8 md:mb-10 gap-2">
          ...
          <div class="header__wrap flex items-center mr-auto">
            <NuxtLink to="/dashboard" class="header__logo inline-flex w-32 mr-4 md:mr-16">
          ...
          <div class="header__control flex items-center gap-2 md:gap-4 relative z-[100]">
            ...
            <div class="notification relative">
              ...
              >
                ...
                <span
                  v-if="hasActivity"
                  class="absolute top-1.5 right-1.5 w-2 h-2 bg-emerald-500 rounded-full border-2 border-[#1E40AF] animate-pulse"
                ></span>
              </button>
            </div>
            ...
            <div class="dropdown user-dropdown relative">
              ...
              >
                ...
                <div
                  v-else
                  class="w-full h-full flex items-center justify-center font-bold text-sm bg-gradient-to-br from-amber-400 to-amber-600 text-primary-900"
                >
                  {{ userInitials }}
                </div>
              </button>
                ...
                >
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <div
                        class="w-12 h-12 rounded-full overflow-hidden border-2 flex-shrink-0"
        ...
        <!-- Security Alert (auto-hides if 2FA enabled) -->
        <Transition
          enter-active-class="transition-all duration-300"
      ...
      >
        <div class="flex flex-col h-full p-6">
          ...
          <div class="flex items-center justify-between mb-8">
            <NuxtLink to="/dashboard" class="inline-flex w-24">
              <img src="/logo1.png" alt="CoinSquare Wealth" class="w-full" />
            </NuxtLink>
            <button
          ...
          <!-- User Card -->
          <div
            class="flex items-center gap-3 p-3 rounded-xl mb-4 bg-white/[0.06] border border-white/[0.08]"
          >
            <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-amber-500/40 flex-shrink-0">
              <img
                v-if="userAvatar"
                :src="userAvatar"
                alt="Profile"
...
// ─────────────────────────────────────────────────────────────
// SECURITY ALERT (auto-dismissible + auto-hides if 2FA on)
// ─────────────────────────────────────────────────────────────
const alertDismissed = ref(false)
const has2FA = computed(() => authStore.state.user?.twoFactorVerification === true)

const showSecurityAlert = computed(() => {
  return !alertDismissed.value && !has2FA.value && authStore.state.isAuthenticated
})
...
onMounted(async () => {
  ...
  // Refresh user from server to get latest avatar / 2FA / KYC status
...
.page__nav .page__link.router-link-active {
