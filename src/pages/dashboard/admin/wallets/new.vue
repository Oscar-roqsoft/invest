<template>
    <DashboardLayout>
      <div class="mb-6 md:mb-8 mt-[70px]">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="font-bold text-2xl md:text-3xl text-gray-800 dark:text-white">New Deposit Wallet</h1>
            <p class="text-xs md:text-sm text-gray-500 dark:text-white/40">
              Add a new currency/network users can deposit to
            </p>
          </div>
        </div>
      </div>
  ...
  const handleSubmit = async (payload) => {
    const res = await store.createWallet(payload)
