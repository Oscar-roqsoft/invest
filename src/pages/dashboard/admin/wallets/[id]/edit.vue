<template>
    <DashboardLayout>
      <div class="mb-6 md:mb-8 mt-[70px]">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="font-bold text-2xl md:text-3xl text-gray-800 dark:text-white">Edit Wallet</h1>
            <p class="text-xs md:text-sm text-gray-500 dark:text-white/40">
              Update the address, limits, or confirmations
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- Loading -->
      <div v-if="!initialWallet" class="space-y-3">
        <div
          v-for="i in 3"
          :key="i"
      ...
      <AdminSystemWalletForm
        v-else
        :initial="initialWallet"
        submit-label="Save Changes"
        :submitting="store.state.isSubmitting"
        @cancel="navigateTo('/dashboard/admin/wallets')"
        @submit="handleSubmit"
      />
    </DashboardLayout>
  </template>
  ...
  onMounted(async () => {
    ...
    let wallet = store.findWalletById(route.params.id)
    if (!wallet || !store.state.walletsLoaded) {
      await store.fetchWallets()
      wallet = store.findWalletById(route.params.id)
    }
  
    if (!wallet) {
      navigateTo('/dashboard/admin/wallets')
      return
    }
  
    initialWallet.value = wallet
  })
  
  const handleSubmit = async (payload) => {
    const res = await store.updateWallet(route.params.id, payload)
    if (res.success) navigateTo('/dashboard/admin/wallets')
  }
</script>