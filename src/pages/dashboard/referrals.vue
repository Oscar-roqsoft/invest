<template>
  <DashboardLayout>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs md:text-sm text-gray-500 dark:text-white/40">
              Invite friends and earn {{ commissionRate }}% commission on every deposit
            </p>
          </div>
        </div>
        <button
          ...
          class="flex items-center gap-2 px-4 md:px-6 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/30 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
    ...
    <!-- Loading State -->
    <div v-if="isLoading && !info" class="py-20 text-center">
      <div class="candles-loader inline-flex gap-2 justify-center">
        <div v-for="i in 4" :key="i" class="candle"></div>
      </div>
      <p class="text-sm text-gray-400 dark:text-white/40 mt-3">Loading referral data...</p>
    </div>
          ...
          <div class="flex flex-wrap items-center justify-between gap-6">
            <div class="max-w-xl">
              ...
              <p class="text-white/60 text-sm md:text-base mb-4">
                Share your unique referral link with friends and earn {{ commissionRate }}% commission on every deposit they make — forever!
              </p>
            ...
            <div class="flex flex-col items-center">
              <div class="w-32 h-32 md:w-40 md:h-40 rounded-2xl bg-white/[0.06] border border-white/[0.1] backdrop-blur-sm flex flex-col items-center justify-center relative overflow-hidden">
                ...
                <span class="text-2xl md:text-3xl font-bold text-amber-400 font-mono relative tracking-widest">
                  {{ referralCode || '—' }}
                </span>
                <button
                  @click="copyReferralCode"
                  class="mt-2 text-xs text-white/50 hover:text-amber-400 relative transition-colors"
                >
                  <i class="bi bi-copy mr-1"></i> Copy Code
                </button>
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div class="lg:col-span-2 space-y-4 md:space-y-6">
          ...
          <div class="rounded-2xl overflow-hidden
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 p-5 md:p-6 border-b border-gray-100 dark:border-white/[0.06]">
              <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-2xl flex items-center justify-center
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-400 dark:text-white/40">
                    {{ stats?.totalReferrals || 0 }} people joined using your link
                  </p>
                </div>
              </div>
            ...
            <!-- Empty -->
            <div v-else-if="!referrals.length" class="flex flex-col items-center justify-center py-16 px-4">
              ...
              <button
                ...
                style="background: linear-gradient(135deg, #F5D77F 0%, #E6BB5C 25%, #D4A44A 50%, #E6BB5C 75%, #F5D77F 100%); background-size: 200% auto; color: #020862;"
              >
                <i class="bi bi-copy mr-1"></i> Copy Referral Link
              </button>
            </div>
              ...
              <table class="w-full min-w-[700px]">
                <thead class="border-b border-gray-100 dark:border-white/[0.06] bg-gray-50/50 dark:bg-white/[0.01]">
                  <tr>
                    ...
                    <th class="text-left py-3 px-5 md:px-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-white/40">Commission</th>
                  ...
                  >
                      ...
                      <div class="flex items-center gap-3">
                        <div
                          class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold
                                 bg-gradient-to-br from-amber-400 to-amber-600 text-primary-900"
                        >
                    ...
                    <td class="py-4 px-5 md:px-6">
                      <div class="text-sm font-bold text-gray-800 dark:text-white">
                        ${{ formatNumber(referral.totalDeposits) }}
                      </div>
                    </td>
                    <td class="py-4 px-5 md:px-6">
                      <div class="text-sm font-bold text-amber-500">
                        +${{ formatNumber(referral.commissionEarned) }}
                      </div>
                    </td>
                    <td class="py-4 px-5 md:px-6">
                      <span
                      ...
                      >
                        <span
                          ...
                          :class="statusDotClass(referral.status)"
                        ></span>
            ...
            <!-- Pagination -->
            <div
              v-if="referralsPagination.pages > 1"
              class="flex flex-wrap items-center justify-between gap-3 p-4 border-t border-gray-100 dark:border-white/[0.06]"
            >
              <span class="text-xs text-gray-500 dark:text-white/40">
                Page {{ referralsPagination.page }} of {{ referralsPagination.pages }}
              </span>
              <div class="flex items-center gap-2">
                <button
                  @click="loadReferrals(referralsPagination.page - 1)"
                  :disabled="referralsPagination.page === 1"
                  class="w-9 h-9 rounded-lg flex items-center justify-center transition-colors
                         disabled:opacity-40 disabled:cursor-not-allowed
                         hover:bg-gray-100 dark:hover:bg-white/[0.06]"
                >
                  <i class="bi bi-chevron-left text-gray-600 dark:text-white/60"></i>
                </button>
                <button
                  @click="loadReferrals(referralsPagination.page + 1)"
                  :disabled="referralsPagination.page === referralsPagination.pages"
                  class="w-9 h-9 rounded-lg flex items-center justify-center transition-colors
                         disabled:opacity-40 disabled:cursor-not-allowed
                         hover:bg-gray-100 dark:hover:bg-white/[0.06]"
                >
                  <i class="bi bi-chevron-right text-gray-600 dark:text-white/60"></i>
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl overflow-hidden
                      bg-white border border-gray-100 shadow-sm
                      dark:bg-[#101A47] dark:border-white/[0.05]">
            <div class="flex items-center justify-between gap-3 p-5 md:p-6 border-b border-gray-100 dark:border-white/[0.06]">
              <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-2xl flex items-center justify-center
                            bg-emerald-500/10 dark:bg-emerald-500/[0.08]">
                  <i class="bi bi-cash-stack text-emerald-500 text-lg"></i>
                </div>
                <div>
                  <h3 class="font-bold text-lg text-gray-800 dark:text-white">Earnings History</h3>
                  <p class="text-xs text-gray-400 dark:text-white/40">
                    Total earned: ${{ formatNumber(earningsTotal) }}
                  </p>
                </div>
              </div>
            </div>

            <div v-if="!earnings.length" class="py-10 text-center px-4">
              <p class="text-sm text-gray-500 dark:text-white/40">No earnings yet</p>
            </div>

            <div v-else class="divide-y divide-gray-100 dark:divide-white/[0.04] max-h-96 overflow-y-auto">
              <div
                v-for="e in earnings"
                :key="e._id"
                class="flex items-center gap-3 p-4 hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors"
              >
                ...
                <div class="flex-1 min-w-0">
                  <p class="font-semibold text-sm text-gray-800 dark:text-white truncate">
                    Commission from {{ e.referred?.name || 'a referral' }}
                  </p>
                  <p class="text-xs text-gray-400 dark:text-white/30">
                    {{ formatDate(e.createdAt) }}
                    <span v-if="e.deposit?.reference"> • {{ e.deposit.reference }}</span>
                  </p>
                </div>
                <div class="text-right">
                  <p class="font-bold text-emerald-500">+${{ formatNumber(e.amount) }}</p>
                  <p class="text-xs text-gray-400 dark:text-white/30">
                    {{ (e.rate * 100).toFixed(0) }}% of ${{ formatNumber(e.sourceAmount) }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="rounded-2xl p-6 relative overflow-hidden
            ...
            <div class="relative z-10">
              ...
              <div class="text-3xl md:text-4xl font-bold text-white mb-2">
                ${{ formatNumber(info?.totalEarnings || 0) }}
              </div>
                ...
                <div class="flex-1">
                  ...
                  <div class="h-1.5 rounded-full bg-gray-100 dark:bg-white/[0.06] overflow-hidden">
                    <div
                      ...
                      :style="{
                        width: `${Math.min((totalReferrals / milestone.count) * 100, 100)}%`,
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <ul class="text-xs text-gray-600 dark:text-white/50 space-y-1.5">
                  <li v-for="term in terms" :key="term" class="flex items-start gap-2">
...
// COMPUTED
// ─────────────────────────────────────────────────────────────
const state = computed(() => referralStore.state)
const info = computed(() => state.value.info)
const stats = computed(() => state.value.stats)
const referrals = computed(() => state.value.referrals)
const earnings = computed(() => state.value.earnings)
const earningsTotal = computed(() => state.value.earningsTotal)
const isLoading = computed(() => state.value.isLoading)

const referralsPagination = computed(() => state.value.referralsPagination)

const referralCode = computed(() => info.value?.referralCode || '')
const referralLink = computed(() => info.value?.referralLink || '')
const commissionRate = computed(() => info.value?.commissionRate || 5)
const totalReferrals = computed(() => info.value?.totalReferrals || 0)

// Stats Cards (derived from info + stats)
const statCards = computed(() => [
  {
    label: 'Total Referrals',
    value: String(info.value?.totalReferrals || 0),
    icon: 'bi bi-people-fill',
    iconColor: '#3b82f6',
    bgColor: 'rgba(59, 130, 246, 0.15)',
    change: `${info.value?.activeReferrals || 0} active`,
  ...
  },
  {
    ...
    change: `${stats.value?.activeReferrals || 0} deposited`,
    changeColor: 'text-emerald-500',
    changeIcon: 'bi bi-person-check',
  },
  {
    label: 'Total Earnings',
    value: `$${formatNumber(info.value?.totalEarnings || 0)}`,
    icon: 'bi bi-cash-stack',
    iconColor: '#bb914a',
    bgColor: 'rgba(187, 145, 74, 0.15)',
    change: `${stats.value?.commissionCount || 0} commissions`,
    changeColor: 'text-emerald-500',
    changeIcon: 'bi bi-arrow-up-short',
  },
  {
    label: 'Pending Payout',
    value: `$${formatNumber(info.value?.pendingEarnings || 0)}`,
    icon: 'bi bi-clock-history',
    iconColor: '#f59e0b',
    bgColor: 'rgba(245, 158, 11, 0.15)',
    change: info.value?.pendingEarnings > 0 ? 'Processing' : 'None pending',
    changeColor: info.value?.pendingEarnings > 0 ? 'text-amber-500' : 'text-gray-400',
...
const milestones = computed(() => {
  const count = totalReferrals.value
  const tiers = [
    { count: 5, reward: '$50 Bonus' },
    { count: 10, reward: '$150 Bonus' },
    { count: 25, reward: '$500 Bonus' },
    { count: 50, reward: '$1,500 Bonus' },
  ]
  return tiers.map((t) => ({
    ...t,
    unlocked: count >= t.count,
  }))
})
...
const howItWorks = [
  ...
  {
    title: 'Earn Forever',
    description: `Get ${commissionRate.value}% commission on every deposit your referred friend makes — for life!`,
    icon: 'bi bi-cash-coin',
  },
]

const terms = [
  `You earn ${commissionRate.value}% commission on every deposit your referrals make`,
  'Commissions are credited instantly and can be withdrawn anytime',
  'Self-referrals are not permitted and will be rejected',
  'Referral rewards are subject to fraud prevention checks',
]
...
const shareOptions = [
  {
    ...
    color: '#000000',
    url: 'https://twitter.com/intent/tweet?text=Join%20me%20on%20CoinSquare%20Wealth&url=',
  },
  {
    name: 'Facebook',
    icon: 'bi bi-facebook',
  ...
  },
  {
    ...
    icon: 'bi bi-whatsapp',
    color: '#25D366',
    url: 'https://wa.me/?text=Join%20me%20on%20CoinSquare%20Wealth%20',
  },
  {
    name: 'Telegram',
    icon: 'bi bi-telegram',
  ...
  },
  {
    ...
    color: '#EA4335',
    url: 'mailto:?subject=Join CoinSquare Wealth&body=',
  },
]
...
const loadReferrals = async (page = 1) => {
  ...
  await referralStore.fetchReferrals(params)
}

// Watch filter changes → reload referrals
watch(statusFilter, () => {
  loadReferrals(1)
})
...
onMounted(async () => {
  ...
  await Promise.all([
    referralStore.fetchInfo(),
    referralStore.fetchStats(),
    loadReferrals(1),
    referralStore.fetchEarnings({ limit: 10 }),
    referralStore.fetchMonthlyEarnings(6),
  ])
})
...
const statusBadgeClass = (status) => {
  switch (status) {
    case 'active':
      return 'bg-emerald-500/10 text-emerald-500 dark:bg-emerald-500/[0.08] dark:text-emerald-400'
    case 'pending':
      return 'bg-amber-500/10 text-amber-500 dark:bg-amber-500/[0.08] dark:text-amber-400'
    default:
      return 'bg-gray-100 text-gray-500 dark:bg-white/[0.06] dark:text-white/40'
  }
}
...
.candle:nth-child(2) { animation-delay: -0.75s; }
.candle:nth-child(3) { animation-delay: -0.5s; }
.candle:nth-child(4) { animation-delay: -0.25s; }
